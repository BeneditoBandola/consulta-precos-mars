...
st.markdown("<h2 style='text-align: center; color: #0F172A; margin-bottom: 2px;'>🍫 Consulta Preços - Mars</h2>", unsafe_allow_html=True)
st.markdown("<p style='text-align: center; font-size: 13px; color: #64748B;'>Digite os <b>dígitos finais do código de barras (EAN)</b> ou o <b>nome do produto</b>:</p>", unsafe_allow_html=True)

codigo_busca = st.text_input("🔍 Buscar Produto:", placeholder="Ex: 79607, whiskas, pedigree, lata...", label_visibility="collapsed")
...
if df_produtos is None:
    ...
    if not df_match.empty:
        ...
        for index, row in df_match.iterrows():
            ...
            caminho_img = obter_caminho_imagem(ean_val) or obter_caminho_imagem(cod_minassal) or obter_caminho_imagem(sku_val)
            ...
            if preco_recomendado > 0:
                ...
                bloco_preco = '<div style="margin-top: 12px;"><span style="color: #D97706; font-size: 13px; font-weight: 700;">⚠️ Preço não cadastrado</span></div>'

            html_card = f'<div class="caixa-produto-info"><span class="badge-familia">{familia_val}</span><h3 style="color: #0F172A; margin-top: 4px; margin-bottom: 6px; font-size: 19px;">{nome_produto}</h3><p style="font-size: 12.5px; color: #64748B; margin-bottom: 0;">{detalhes_str}</p>{bloco_preco}</div>'
